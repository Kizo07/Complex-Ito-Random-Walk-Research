...
\pandocbounded{\includegraphics[keepaspectratio]{figures/f01_occupation.png}}
...
\begin{longtable}[]{@{}lll@{}}
\caption{Reduced-grid errors against the reference
...
\bottomrule\noalign{}
\endlastfoot
PPA (\(m=10\), pinned) & \textbf{0.0079} & 0.054 \\
Phase 15 spiral (collocated) & 0.0066 & \textbf{0.039} \\
...
(Section~\ref{sec-scope}). The extended 90-configuration grid
(\(\sigma\) up to \(0.6\), five spots) gives PPA RMSE \(0.016\) / MAE
\(0.167\) versus \(0.285/1.143\) for BAW (Table~\ref{tbl-extended}).

\begin{figure}[tbp]

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{figures/f02_ppa.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{figures/f03_benchmark.png}}

}

\caption{\label{fig-benchmark}RMSE/MAE against the reference solver on
the reduced grid.}
...
\begin{longtable}[]{@{}lll@{}}
\caption{Extended 90-configuration benchmark against the reference
...
\bottomrule\noalign{}
\endlastfoot
PPA (\(m=10\), pinned) & 0.0159 & \textbf{0.167} \\
Phase 15 spiral (collocated) & \textbf{0.0158} & 0.181 \\
...
\pandocbounded{\includegraphics[keepaspectratio]{figures/f04_decay.png}}
...
\item
  \textbf{Reference uncertainty at long maturity.} The sequential
  level-set reference solver accumulates small numerical differences
  over its \(\sim250\) causal steps: boundary values at \(T=3\) differ
  at the \(10^{-2}\)--\(10^{-1}\) scale across environments (e.g.~numpy
  SIMD rounding), and prices at the \(10^{-2}\) scale (stationarity
  keeps this one-to-two orders below the boundary differences). At
  \(T=3\) the reference-vs-CRR spread is of the same order as the
  formula errors, so the \(T=3\) MAE entries in
  Table~\ref{tbl-reduced}--Table~\ref{tbl-extended} mix formula error
