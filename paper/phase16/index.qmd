...
| Method | RMSE | MAE |
|--------|------|-----|
| PPA ($m=10$, pinned) | **0.0079** | 0.054 |
| Phase 15 spiral (collocated) | 0.0066 | **0.039** |
| Barone-Adesi–Whaley | 0.164 | 0.437 |

: Reduced-grid errors against the reference solver. {#tbl-reduced}

The PPA formula matches the Phase 15 spiral in RMSE and removes its
quadrature entirely; its MAE is dominated by the $T=3$ configurations,
where the reference boundary itself is uncertain at the same order
(@sec-scope). The extended 90-configuration grid
($\sigma$ up to $0.6$, five spots) gives PPA RMSE $0.016$ / MAE $0.167$
versus $0.285/1.143$ for BAW (@tbl-extended).
...
![RMSE/MAE against the reference solver on the reduced grid.](figures/f03_benchmark.png){#fig-benchmark}

## Extended grid {#sec-extended}

| Method | RMSE | MAE |
|--------|------|-----|
| PPA ($m=10$, pinned) | 0.0159 | **0.167** |
| Phase 15 spiral (collocated) | **0.0158** | 0.181 |
| Barone-Adesi–Whaley | 0.285 | 1.143 |

: Extended 90-configuration benchmark against the reference solver. {#tbl-extended}
...
- **Reference uncertainty at long maturity.** The sequential level-set
  reference solver accumulates small numerical differences over its
  $\sim250$ causal steps: boundary values at $T=3$ differ at the
  $10^{-2}$–$10^{-1}$ scale across environments (e.g. numpy SIMD
  rounding), and prices at the $10^{-2}$ scale (stationarity keeps this
  one-to-two orders below the boundary differences). At $T=3$ the
  reference-vs-CRR spread is of the same order as the formula errors, so
  the $T=3$ MAE entries in @tbl-reduced–@tbl-extended mix formula error
  and reference uncertainty; for $T\le1$ all formula errors are
  $\le0.013$.
